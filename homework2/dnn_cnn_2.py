...
def main(main_params):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ################################################################################
            # aaTODO (2): Connect the three modules for the forward pass                     #
            # model['C2']                                                                  #
            # model['nonlinear2']                                                          #
            # model['M2']                                                                  #
            ...
            ################################################################################
            zzz = model['C2'].forward(m1)
            zzzz= model['nonlinear2'].forward(zzz)
            m2 = model['M2'].forward(zzzz)
            # TODO (2)
            ...
            ################################################################################
            # aaTODO (3): Connect the three modules for the backward pass                    #
            # model['C2']                                                                  #
            # model['nonlinear2']                                                          #
            # model['M2']                                                                  #
            ...
            ################################################################################
            grad_zzzz = model['M2'].backward(zzzz, grad_m2)
            grad_zzz = model['nonlinear2'].backward(zzz,grad_zzzz)
            grad_m1 = model['C2'].backward(m1,grad_zzz)
            # TODO (3)
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ################################################################################
            # aaTODO (4): Connect the three modules for the forward pass                     #
            # model['C2']                                                                  #
            # model['nonlinear2']                                                          #
            # model['M2']                                                                  #
            # into the forward pass.                                                       #
        ...
        for i in range(int(np.floor(N_val / minibatch_size))):
            ...
            ################################################################################
            # aaTODO (5): Connect the three modules for the forward pass                     #
            # model['C2']                                                                  #
            # model['nonlinear2']                                                          #
            # model['M2']                                                                  #
            # into the forward pass.                                                       #
