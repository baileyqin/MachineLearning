...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        # aaTODO: Implement the linear forward pass. Store the result in forward_output  #
        ################################################################################
        ...
        #for i in range(0,n):
        #    forward_output[i] = np.matmul( np.transpose(self.params['W']), X[i] ) + self.params['b']
    ...
    def backward(self, X, grad):
        ...
        ##########################################################################################################################
        # aaTODO: Implement the backward pass (i.e., compute the following three terms)                                            #
        ...
        self.gradient['b']= np.matmul(np.transpose(ones), grad)
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        # aaTODO: Implement the relu forward pass. Store the result in forward_output    #
        ################################################################################
        zeros = np.zeros(X.shape)
        forward_output = np.maximum(zeros, X)
        return forward_output
    ...
    def backward(self, X, grad):
        ...
        ##########################################################################################################################
        # aaTODO: Implement the backward pass (i.e., compute the following term)                                                   #
        ...
        A[A<0] = 0
        A[A>0] = 1
        backward_output = np.multiply(A, grad)
        #for i in range(0,n):
        #	for j in range(0,m):
        #		if X[i,j]>0:
...
class dropout:
    ...
    def forward(self, X, is_train):
        ...
        ################################################################################
        #  aaTODO: We provide the forward pass to you. You only need to understand it.   #
        ################################################################################
