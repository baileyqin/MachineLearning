...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    for i in range(N):
        class_prob[Y[i]] += 1
    for i in range(num_classes):
        class_prob[i] = class_prob[i] / N

    total0words = alpha * vocab_size
    total1words = alpha * vocab_size

    for n in range(N):
        if Y[n] == 0:
            for i in range(len(X[n])):
                class_word_prob[0][X[n][i][0]] += X[n][i][1]
                total0words += X[n][i][1]
        else:
            for i in range(len(X[n])):
                class_word_prob[1][X[n][i][0]] += X[n][i][1]
                total1words += X[n][i][1]
    for i in range(vocab_size):
        class_word_prob[0][i] = class_word_prob[0][i] / float(total0words)
        class_word_prob[1][i] = class_word_prob[1][i] / float(total1words)
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    logpi0 = np.log(class_prob[0])
    logpi1 = np.log(class_prob[1])

    logtheta0k = np.zeros(N)
    logtheta1k = np.zeros(N)

    for n in range(N):
        for i in range(len(X[n])):
            if class_word_prob[0][X[n][i][0]] == 0:
                logtheta0k[n] += X[n][i][1] * -1.0e50
            else:
                logtheta0k[n] += X[n][i][1] * np.log(class_word_prob[0][X[n][i][0]])
    
    for n in range(N):
        for i in range(len(X[n])):
            if class_word_prob[1][X[n][i][0]] == 0:
                logtheta1k[n] += X[n][i][1] * -1.0e50
            else:
                logtheta1k[n] += X[n][i][1] * np.log(class_word_prob[1][X[n][i][0]])
    for n in range(N):    
        compare = logpi0 + logtheta0k[n] - (logpi1 + logtheta1k[n])
        if compare > 0:
            Ypred.append(0)
        else:
            Ypred.append(1)
